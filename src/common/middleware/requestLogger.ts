...
const requestLogger = (options?: Options): RequestHandler[] => {
  ...
  return [loggerMiddleware, responseBodyMiddleware, pinoHttp(pinoOptions)];
};
...
const loggerMiddleware: RequestHandler = (req, res, next) => {
  ...
  res.on('finish', () => logger.info(`[${new Date().toISOString()}] ${req.method} ${req.originalUrl} ${res.statusCode} - ${Date.now() - start}ms`));
  next();
};
